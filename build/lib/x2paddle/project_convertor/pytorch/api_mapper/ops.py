    ...
    def run(self):
        ...
        if self.pytorch_api_name == "torch.clip":
            if self.rename_func_name("x2paddle.torch2paddle.clip"):
                return [], generate_api_code(self.func_name, self.args,
                                             self.kwargs), []
        if self.pytorch_api_name == "torch.cos":
            if self.rename_func_name("x2paddle.torch2paddle.cos"):
                return [], generate_api_code(self.func_name, self.args,
                                             self.kwargs), []
            
        if self.pytorch_api_name == "torch.sin":
            if self.rename_func_name("x2paddle.torch2paddle.sin"):
                return [], generate_api_code(self.func_name, self.args,
                                             self.kwargs), []
